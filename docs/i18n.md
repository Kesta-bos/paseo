...
English source strings live in `packages/app/src/i18n/resources/en.ts`. Each supported locale has a sibling file in `packages/app/src/i18n/resources/` (e.g. `zh-CN.ts` for Simplified Chinese, `ko.ts` for Korean).
...
The parity test catches missing keys between English and every other locale's resources.
...
- Batch 5A added Korean (`ko`) as a supported locale with full client-owned UI resource coverage in `resources/ko.ts`, wired through `locales.ts` (language option, native name, language-name matrix, and `ko`/`ko-*` system-locale resolution) and registered in `i18next.ts`. Key parity, fallback-ratio, and interpolation-placeholder tests cover Korean, and the settings-i18n e2e exercises switching to and from Korean. Korean is LTR and needs no font changes (system fonts cover Hangul).
